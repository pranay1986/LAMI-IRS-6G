...
9. Save output CSV files and generated plots in `results/` and `figures/`.
...
After running the workflow, the following figures are generated:

| Figure | Output path | Purpose |
|---|---|---|
| Latency and energy comparison | `figures/latency_energy_comparison.png` | Reproduces the controller-side energy/latency comparison |
| SNR recovery | `figures/snr_recovery.png` | Shows transient recovery under IRS faults and interference bursts |
| Distance adaptation | `figures/distance_adaptation.png` | Shows SNR behavior under increasing transmitter–receiver distance |
| Scalability latency | `figures/scalability_latency.png` | Estimates controller-side latency under larger IRS panels |
| Scalability memory | `figures/scalability_memory.png` | Estimates descriptor/control memory growth |
...
figures/
├── latency_energy_comparison.png
├── snr_recovery.png
├── distance_adaptation.png
